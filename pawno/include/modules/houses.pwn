...
CMD:buyhouse(playerid, params[]) {
	if(playerInfo[playerid][pHouse] != 0) return SCM(playerid, COLOR_ERROR, eERROR"Ai deja o casa cumparata.");
	...
	if(playerInfo[playerid][areaHouse] != 0 && IsPlayerInRangeOfPoint(playerid, 3.5, houseInfo[playerInfo[playerid][areaHouse]][hExtX], houseInfo[playerInfo[playerid][areaHouse]][hExtY], houseInfo[playerInfo[playerid][areaHouse]][hExtZ])) {
		if(houseInfo[playerInfo[playerid][areaHouse]][hPrice] == 0) return SCM(playerid, COLOR_ERROR, eERROR"Aceasta casa nu este de vanzare.");
...
CMD:sellhousestate(playerid, params[]) {
	...
	if(GetPlayerVirtualWorld(playerid) != 0 && GetPlayerInterior(playerid) != 0 && playerInfo[playerid][pinHouse] != -1) return SCM(playerid, COLOR_ERROR, eERROR"Nu poti folosi aceasta comanda deoarece, esti intr-un alt virtualworld / interior / esti intr-o casa.");
	if(playerInfo[playerid][pHouse] == 0 && playerInfo[playerid][pHouseID] == -1) return SCM(playerid, COLOR_ERROR, eERROR"Nu detii o afacere.");
...
CMD:housebalance(playerid, params[]) {
	if(!isPlayerLogged(playerid)) return SCM(playerid, COLOR_ERROR, eERROR"Nu esti logat, pentru a face aceasta actiune.");
	if(playerInfo[playerid][pHouse] == 0 && playerInfo[playerid][pHouseID] == -1) return SCM(playerid, COLOR_ERROR, eERROR"Nu detii o casa.");
...
CMD:housewithdraw(playerid, params[]) {
	if(!isPlayerLogged(playerid)) return SCM(playerid, COLOR_ERROR, eERROR"Nu esti logat, pentru a face aceasta actiune.");
	if(playerInfo[playerid][pHouse] == 0 && playerInfo[playerid][pHouseID] == -1) return SCM(playerid, COLOR_ERROR, eERROR"Nu detii o casa.");
	if(playerInfo[playerid][pinHouse] != playerInfo[playerid][pHouseID]) return SCM(playerid, COLOR_ERROR, eERROR"Poti folosi aceasta comanda doar din interiorul casei tale.");
	extract params -> new suma; else {
		SCMf(playerid, COLOR_GREY, "* House Notice: Balanta ta la casa este de $%s.", formatNumber(houseInfo[playerInfo[playerid][pHouseID]][hBalance]));
		return sendPlayerSyntax(playerid, "/housewithdraw <money>");
	}
	if(houseInfo[playerInfo[playerid][pHouseID]][hBalance] < suma) return SCM(playerid, COLOR_ERROR, eERROR"Nu ai suma aceasta de bani in balanta casei tale.");
...
CMD:housedeposit(playerid, params[]) {
	...
	if(!PlayerMoney(playerid, suma)) return SCM(playerid, COLOR_ERROR, eERROR"Nu ai suma aceasta de bani, pentru a adauga in balanta casei tale.");
	houseInfo[playerInfo[playerid][pHouseID]][hBalance] += suma;
...
CMD:sellhouse(playerid, params[]) {
	...
	if(!(1 <= suma <= 999999999)) return SCM(playerid, COLOR_ERROR, eERROR"Invalid money ($1 - $999,999,999).");
...
CMD:adminhouse(playerid, params[]) {
	...
	if(!Iter_Contains(ServerHouses, house)) return SCM(playerid, COLOR_ERROR, eERROR"Acesta casa nu exista.");	
...
CMD:rentabil(playerid, params[]) {
	if(!isPlayerLogged(playerid)) return SCM(playerid, COLOR_ERROR, eERROR"Nu esti logat, pentru a face aceasta actiune.");
	if(playerInfo[playerid][pHouse] == 0 && playerInfo[playerid][pHouseID] == -1) return SCM(playerid, COLOR_ERROR, eERROR"Nu detii o casa.");
	if(GetPVarInt(playerid, "RentDeelay") > gettime()) return SCMf(playerid, COLOR_ERROR, eERROR"Trebuie sa astepti %d secunde inainte sa folosesti aceasta comanda.", (GetPVarInt(playerid, "RentDeelay") - gettime()));
...
CMD:rentroom(playerid, parmas[]) {
	if(!isPlayerLogged(playerid)) return SCM(playerid, COLOR_ERROR, eERROR"Nu esti logat, pentru a face aceasta actiune.");
	if(playerInfo[playerid][pHouse] == 1 && playerInfo[playerid][pHouseID] != -1) return SCM(playerid, COLOR_ERROR, eERROR"Detii deja o casa, nu poti folosi aceasta comanda.");
	if(playerInfo[playerid][pRent] != -1) return SCM(playerid, COLOR_ERROR, eERROR"Ai deja rentroom la o casa, foloseste (/unrentroom).");
	if(playerInfo[playerid][areaHouse] != 0 && IsPlayerInRangeOfPoint(playerid, 3.5 , houseInfo[playerInfo[playerid][areaHouse]][hExtX], houseInfo[playerInfo[playerid][areaHouse]][hExtY], houseInfo[playerInfo[playerid][areaHouse]][hExtZ])) {
		if(houseInfo[playerInfo[playerid][areaHouse]][hRentabil] == 0) return SCM(playerid, COLOR_ERROR, eERROR"Aceasta casa nu este rentabila.");
		if(houseInfo[playerInfo[playerid][areaHouse]][hPrice] > 0) return SCM(playerid, COLOR_ERROR, eERROR"Nu mai poti da rent, deoarece casa este de vanzare.");
		if(houseInfo[playerInfo[playerid][areaHouse]][hOwned] == 0) return SCM(playerid, COLOR_ERROR, eERROR"Nu poti da rent, deoarece aceasta casa nu detine un detinator.");
...
CMD:unrentroom(playerid, params[]) {
	if(!isPlayerLogged(playerid)) return SCM(playerid, COLOR_ERROR, eERROR"Nu esti logat, pentru a face aceasta actiune.");
	if(playerInfo[playerid][pRent] == -1) return SCM(playerid, COLOR_ERROR, eERROR"Nu ai rentroom la o casa, foloseste (/rentroom).");
...
CMD:setrentprice(playerid, params[]) {
	if(!isPlayerLogged(playerid)) return SCM(playerid, COLOR_ERROR, eERROR"Nu esti logat, pentru a face aceasta actiune.");
	if(playerInfo[playerid][pHouse] == 0 && playerInfo[playerid][pHouseID] == -1) return SCM(playerid, COLOR_ERROR, eERROR"Nu detii o casa.");
	if(houseInfo[playerInfo[playerid][pHouseID]][hRentabil] == 0) return SCM(playerid, COLOR_ERROR, eERROR"Nu poti folosi aceasta comanda, deoarece casa ta nu are rent-ul activat.");
	extract params -> new suma; else return sendPlayerSyntax(playerid, "/setrentprice <money>");
	if(!(1 <= suma <= 5000)) return SCM(playerid, COLOR_ERROR, eERROR"Invalid money ($1 - $5,000).");
...
CMD:upgradehouse(playerid, params[]) {
	...
	if(upgrade == 1) {
		if(houseInfo[playerInfo[playerid][pHouseID]][hUpgrade] == 1) return SCM(playerid, COLOR_ERROR, eERROR"Detii deja acest upgrade heal.");
		if(!PlayerMoney(playerid, 50000)) return SCM(playerid, COLOR_ERROR, eERROR"Nu detii $50,000");
		...
		if(houseInfo[playerInfo[playerid][pHouseID]][hUpgrade] == 2) return SCM(playerid, COLOR_ERROR, eERROR"Detii deja acest upgrade armour.");
		if(!PlayerMoney(playerid, 100000)) return SCM(playerid, COLOR_ERROR, eERROR"Nu detii $100,000");
...
CMD:eat(playerid, params[]) {
	...
	if(playerInfo[playerid][pHouse] != 0 && playerInfo[playerid][pHouseID] != -1) {
		if(playerInfo[playerid][pinHouse] == playerInfo[playerid][pHouseID]){
			if(houseInfo[playerInfo[playerid][pHouseID]][hUpgrade] <= 0) return SCM(playerid, COLOR_ERROR, eERROR"Casa ta nu detine upgrade-ul de heal.");
		...
		if(GetPlayerVirtualWorld(playerid) == houseInfo[playerInfo[playerid][pRent]][hID] && GetPlayerInterior(playerid) == houseInfo[playerInfo[playerid][pRent]][hInterior]) {
			if(houseInfo[playerInfo[playerid][pRent]][hUpgrade] <= 0) return SCM(playerid, COLOR_ERROR, eERROR"Rentul tau nu detine upgrade-ul de heal.");
			SetPlayerHealthEx(playerid, 100);
			sendNearbyMessage(playerid, COLOR_PURPLE, 25.0, "* %s si-a dat heal.", getName(playerid));
		}
	}
...
CMD:armour(playerid, params[]) {
	if(!isPlayerLogged(playerid)) return SCM(playerid, COLOR_ERROR, eERROR"Nu esti logat, pentru a face aceasta actiune.");
	if(playerInfo[playerid][pFaction] != 2 && playerInfo[playerid][pFaction] != 3 && playerInfo[playerid][pFaction] != 4) return SCM(playerid, COLOR_ERROR, eERROR"Aceasta comanda este doar pentru politsti.");
	...
	if(playerInfo[playerid][pHouse] != 0 && playerInfo[playerid][pHouseID] != -1) {
		if(playerInfo[playerid][pinHouse] == playerInfo[playerid][pHouseID]) {
			if(houseInfo[playerInfo[playerid][pHouseID]][hUpgrade] <= 0) return SCM(playerid, COLOR_ERROR, eERROR"Casa ta nu detine upgrade-ul de armour.");
			SetPlayerHealthEx(playerid, 100);
		...
		if(GetPlayerVirtualWorld(playerid) == houseInfo[playerInfo[playerid][pRent]][hID] && GetPlayerInterior(playerid) == houseInfo[playerInfo[playerid][pRent]][hInterior]) {
			if(houseInfo[playerInfo[playerid][pRent]][hUpgrade] <= 0) return SCM(playerid, COLOR_ERROR, eERROR"Rentul tau nu detine upgrade-ul de armour.");
			SetPlayerHealthEx(playerid, 100);
			SetPlayerArmourEx(playerid, 100);
			sendNearbyMessage(playerid, COLOR_PURPLE, 25.0, "* %s si-a dat heal & armour.", getName(playerid));
		}
...
CMD:createhouse(playerid, params[], help) {
	...
	if(Iter_Count(ServerHouses) >= MAX_HOUSES) return SCM(playerid, COLOR_ERROR, eERROR"Database:Limita de case a fost atinsa !");
	...
	if(!(0 <= rent <= 1)) return SCM(playerid, COLOR_ERROR, eERROR"Invalid rent available (0 - 1).");
