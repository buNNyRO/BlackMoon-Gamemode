...
function InsertCarDS(playerid, model) {
	...
	PersonalVeh[idcar][pvID] = cache_insert_id();
	PersonalVeh[idcar][pvOwnerID] = playerInfo[playerid][pSQLID];
	PersonalVeh[idcar][pvModelID] = model;
	PersonalVeh[idcar][pvColorOne] = 1;
	PersonalVeh[idcar][pvColorTwo] = 1;
	PersonalVeh[idcar][pvX] = DebugVehicles[rand][0];
	PersonalVeh[idcar][pvY] = DebugVehicles[rand][1]; 
	PersonalVeh[idcar][pvZ] = DebugVehicles[rand][2];
	PersonalVeh[idcar][pvAngle] = DebugVehicles[rand][3];
	PersonalVeh[idcar][pvVirtualWorld] = 0;
	PersonalVeh[idcar][pvInterior] = 0;
	PersonalVeh[idcar][pvFuel] = 100;
	PersonalVeh[idcar][pvPaintJob] = -1;
	PersonalVeh[idcar][pvLock] = 1;
	PersonalVeh[idcar][pvHealth] = 1000;
	PersonalVeh[idcar][pvInsurancePoints] = 10;
	PersonalVeh[idcar][pvSpawnedID] = INVALID_VEHICLE_ID;
	PersonalVeh[idcar][pvDespawnTime] = 0;
	strmid(PersonalVeh[idcar][pvCarPlate], "DS Car", 0, 6, 255);
	update("UPDATE `server_personal_vehicles` SET `OwnerID` = '%d', `ColorOne` = '1', `ColorTwo` = '1', `X` = '%f', `Y` = '%f', `Z` = '%f', `Angle` = '%f', `CarPlate` = '%s', `InsurancePoints` = '10' WHERE `ID` = '%d'", PersonalVeh[idcar][pvOwnerID], DebugVehicles[rand][0], DebugVehicles[rand][1], DebugVehicles[rand][2], DebugVehicles[rand][3], PersonalVeh[idcar][pvCarPlate], PersonalVeh[idcar][pvID]);
	gString[0] = (EOS);
	va_format(gString, sizeof(gString), "{ccaa2d}* (Dealership){ffffff}: Ti-ai cumparat o masina %s (%d) pentru $%s. Aceasta o poti vedea tastand comanda /vehicles (/v), aceasta nu a fost spawnata pentru a evita lagul.", getVehicleName(PersonalVeh[idcar][pvModelID]), PersonalVeh[idcar][pvModelID], formatNumber(DSModels[SelectedCar[playerid][DealerID[playerid]]][dPrice]));
...
hook OnPlayerClickTextDraw(playerid, Text:clickedid) {
	if(clickedid == serverDealerTD[1]) {
		...
		DestroyVehicle(DealerCar[playerid]);
		DealerCar[playerid] = CreateVehicle(DSModels[DealerID[playerid]][dModel],1524.8397,-2412.6653,24.4380,35.8937,1,1,-1,0); 
		vehPersonal[DealerCar[playerid]] = -1;
		SetVehicleVirtualWorld(DealerCar[playerid], playerid+1);
		if(GetPlayerCash(playerid) < DSModels[SelectedCar[playerid][DealerID[playerid]]][dPrice]) va_PlayerTextDrawSetString(playerid, serverDealerPTD[playerid][2], "Price: ~r~$%s~W~~H~~N~Stock: ~R~%d available~W~~H~~N~Max Speed: ~R~%dKM/H", formatNumber(DSModels[SelectedCar[playerid][DealerID[playerid]]][dPrice]), DSModels[SelectedCar[playerid][DealerID[playerid]]][dStock], getVehicleMaxSpeed(DSModels[SelectedCar[playerid][DealerID[playerid]]][dModel]));
		else if(GetPlayerCash(playerid) >= DSModels[SelectedCar[playerid][DealerID[playerid]]][dPrice]) va_PlayerTextDrawSetString(playerid, serverDealerPTD[playerid][2], "Price: ~g~$%s~W~~H~~N~Stock: ~R~%d available~W~~H~~N~Max Speed: ~R~%dKM/H", formatNumber(DSModels[SelectedCar[playerid][DealerID[playerid]]][dPrice]), DSModels[SelectedCar[playerid][DealerID[playerid]]][dStock], getVehicleMaxSpeed(DSModels[SelectedCar[playerid][DealerID[playerid]]][dModel]));
		va_PlayerTextDrawSetString(playerid, serverDealerPTD[playerid][1], "%s", getVehicleName(DSModels[SelectedCar[playerid][DealerID[playerid]]][dModel]));		
	...
	if(clickedid == serverDealerTD[7]) {
		...
		vehFuel[DealerCar[playerid]] = 100;
