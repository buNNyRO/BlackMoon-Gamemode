CMD:addexamcp(playerid, params[]) {
	...
	if(Iter_Count(ExamenCheckpoints) >= MAX_EXAM_CHECKPOINTS) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Eroare ! Database:Limita de checkpoint-uri a fost atinsa !");
...
CMD:warn(playerid, params[])
{
	if(!Iter_Contains(ServerAdmins, playerid))
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");

	if(GetPVarInt(playerid, "warnDeelay") > gettime()) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa astepti %d secunde inainte sa folosesti aceasta comanda.", (GetPVarInt(playerid, "warnDeelay") - gettime()));	

	extract params -> new player:userID, string:reason[64]; else return sendPlayerSyntax(playerid, "/warn <name/id> <reason>");

	if(!isPlayerLogged(userID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");
...
CMD:unwarn(playerid, params[])
{
	...
	if(GetPVarInt(playerid, "unWarnDeelay") > gettime())
		return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa astepti %d secunde inainte sa folosesti aceasta comanda.", (GetPVarInt(playerid, "unWarnDeelay") - gettime()));
	...
	if(!isPlayerLogged(userID))
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");

	if(playerInfo[userID][pWarn] == 0)
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu are nici-un warn.");
...
CMD:ban(playerid, params[])
{
	...
	if(GetPVarInt(playerid, "banDeelay") > gettime())
		return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa astepti %d secunde inainte sa folosesti aceasta comanda.", (GetPVarInt(playerid, "banDeelay") - gettime()));
	...
	if(playerid == userID)
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti folosi aceasta comanda asupra ta.");

	if(playerInfo[userID][pAdmin] >= playerInfo[playerid][pAdmin])
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti sa-i dai ban acelui jucator.");
...
CMD:banoffline(playerid, params[])
{
	...
	foreach(new i : loggedPlayers)
	{
		if(strmatch(getName(i), playerName))
			return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul este conectat.");
	}
	gQuery[0] = (EOS);
...
CMD:unban(playerid, params[])
{
	...
	if(GetPVarInt(playerid, "unbanDeelay") > gettime())
		return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa astepti %d secunde inainte sa folosesti aceasta comanda.", (GetPVarInt(playerid, "unbanDeelay") - gettime()));
...
CMD:mute(playerid, params[])
{
	...
	if(GetPVarInt(playerid, "muteDeelay") > gettime())
		return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa astepti %d secunde inainte sa folosesti aceasta comanda.", (GetPVarInt(playerid, "muteDeelay") - gettime()));
	...
	if(playerInfo[userID][pAdmin] >= playerInfo[playerid][pAdmin])
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti sa-i dai mute acelui jucator.");

	if(minutes < 1 || minutes > 120)
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Numarul de minute introdus nu este valid (1 - 120).");

	if(playerInfo[userID][pMute] > 0 && playerInfo[playerid][pAdmin] < 2)
		return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja mute.");
...
CMD:unmute(playerid, params[])
{
	...
	if(playerInfo[userID][pMute] < 0)
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu are mute.");
...
CMD:mutedplayers(playerid, params[])
{
	...
	if(!Iter_Count(MutedPlayers))
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu sunt jucatori cu mute.");
...
CMD:setadmin(playerid, params[])
{
	...
	if(admin < 0 || admin > 7) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest level de admin este invalid (0 - 7).");

	if(!isPlayerLogged(userID))
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");

	if(playerInfo[userID][pAdmin] == admin)
		return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja acest nivel de admin.");
...
CMD:sethelper(playerid, params[])
{
	...
	if(playerInfo[userID][pHelper] == helper)
		return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja acest nivel de helper.");
...
CMD:adminsuspendlicense(playerid, params[])
{
	...
	if(hours < 1 || hours > 15)
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Numarul de ore este Invalid (1 - 15).");
	...
	switch(YHash(license))
	{
		case _H<driving>:
		{
			if(playerInfo[userID][pDrivingLicenseSuspend] == hours)
				return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja licenta de condus suspendata pentru %d ore.", hours);
		...
		case _H<flying>:
		{
			if(playerInfo[userID][pFlyLicenseSuspend] == hours)
				return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja licenta de pilot pentru %d ore.", hours);
		...
		case _H<boat>:
		{
			if(playerInfo[userID][pBoatLicenseSuspend] == hours)
				return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja licenta de navigatie pentru %d ore.", hours);
...
CMD:admingivelicense(playerid, params[])
{
	...
	if(hours < 1 || hours > 300)
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Numarul de ore este Invalid (1 - 300).");
	...
	switch(YHash(license))
	{
		case _H<driving>:
		{
			if(playerInfo[userID][pDrivingLicense] == hours)
				return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja licenta de condus pentru %d ore.", hours);

			if(playerInfo[userID][pDrivingLicenseSuspend] > 0 && playerInfo[playerid][pAdmin] < 6)
				return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are licenta de condus suspendata.");
		...
		case _H<flying>:
		{
			...
			if(playerInfo[userID][pFlyLicenseSuspend] > 0 && playerInfo[playerid][pAdmin] < 6)
				return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are licenta de pilot suspendata.");
		...
		case _H<boat>:
		{
			...
			if(playerInfo[userID][pBoatLicenseSuspend] > 0 && playerInfo[playerid][pAdmin] < 6)
				return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are licenta de navigatie suspendata.");
		...
		case _H<all>:
		{
			if((playerInfo[userID][pDrivingLicenseSuspend] || playerInfo[userID][pFlyLicenseSuspend] || playerInfo[userID][pBoatLicenseSuspend] || playerInfo[userID][pWeaponLicenseSuspend]) && playerInfo[playerid][pAdmin] < 6)
				return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are o licenta suspendata.");
	...
	switch(YHash(license))
	{
		case _H<driving>:
		{
			if(playerInfo[userID][pDrivingLicense] == 0)
				return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu are licenta de condus.");
		...
		case _H<flying>:
		{
			if(playerInfo[userID][pFlyLicense] == 0)
				return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu are licenta de pilot.");
		...
		case _H<boat>:
		{
			if(playerInfo[userID][pBoatLicense] == 0)
				return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu are licenta de navigatie.");
		...
		case _H<weapon>:
		{
			if(playerInfo[userID][pWeaponLicense] == 0)
				return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu are licenta de port-arma.");

			if(playerInfo[userID][pWeaponLicenseSuspend] > 0 && playerInfo[playerid][pAdmin] < 6)
				return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are licenta de port-arma suspendata.");
...
CMD:kick(playerid, params[]) {
	...
	if(GetPVarInt(playerid, "kickDeelay") >= gettime()) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa astepti %d secunde inainte de a folosi aceasta comanda.", (GetPVarInt(playerid, "kickDeelay") - gettime()));
	extract params -> new player:id, silent, string:reason[64]; else return sendPlayerSyntax(playerid, "/kick <name/id> <silent> <reason>");
	if(silent < 0 || silent > 1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Silent Invalid. ( 0 = Normal | 1 = Silent )");
	if(!isPlayerLogged(id)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");
	if(id == playerid) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti folosi comanda, asupra ta.");
	if(playerInfo[id][pAdmin] >= playerInfo[playerid][pAdmin] && !strmatch(getName(playerid), "Vicentzo")) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti sa-i dai kick acestui jucator.");
...
CMD:spawncar(playerid, params[]) {
	if(!Iter_Contains(ServerAdmins, playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(Iter_Count(AdminVehicles) >= MAX_ADMIN_VEHICLES) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti crea un vehicul deoarece s-a atins numarul maxim de vehicule (Vehicule Existente: %d | Numar Maxim: %d)", Iter_Count(AdminVehicles), MAX_ADMIN_VEHICLES);
	if(playerInfo[playerid][pDrivingLicense] == 0 && playerInfo[playerid][pFlyLicense] == 0 && playerInfo[playerid][pBoatLicense] == 0) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti spawna o masina deoarece nu ai licenta de condus / navigatie / pilot.");
	...
	if(modelid < 400 || modelid > 611) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid Model ID (400 - 611).");
	if(firstcolor < 0 || firstcolor > 255) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid First Color (0 - 255).");
	if(secondcolor < 0 || secondcolor > 255) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid Second Color (0 - 255).");
...
CMD:despawncar(playerid, params[]) {
	...
	if(!IsPlayerInAnyVehicle(playerid)) {
		...
		if(!IsValidVehicle(strval(params))) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest vehicul nu exista.");
		if(!Iter_Contains(AdminVehicles, strval(params))) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest vehicul nu a fost creat de catre un admin.");
	...
	if(!Iter_Contains(AdminVehicles, vehicleid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest vehicul nu a fost spawnat de un admin.");
...
CMD:despawncars(playerid, params[]) {
	if(!Iter_Contains(ServerAdmins, playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(!Iter_Count(AdminVehicles)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu sunt vehicule create pe server.");
...
CMD:gotocar(playerid, params[]) {
	if(!Iter_Contains(ServerAdmins, playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(IsPlayerInAnyVehicle(playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa cobori jos din vehicul.");
	if(isnull(params) || !IsNumeric(params)) return sendPlayerSyntax(playerid, "/gotocar <vehicle id>");
	if(!IsValidVehicle(strval(params))) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Vehiculul nu exista.");
...
CMD:closestcar(playerid, params[]) {
	...
	if(!IsValidVehicle(vehicleID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu a fost gasit un vehicul in raza ta.");
...
CMD:entercar(playerid, params[]) {
	...
	if(!IsValidVehicle(vehicleid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid vehicle id.");
	if(seatid < 0 || seatid > 4) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid seat id.");
	if(isBike(vehicleid) && seatid > 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel vehicul nu are mai mult de un loc disponibil.");
	if(isMotor(vehicleid) && seatid > 1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel vehicul nu are mai mult de doua locuri disponibile.");
...
CMD:goto(playerid, params[]) {
	...
	if(!isPlayerLogged(id)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");
	if(id == playerid) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti folosi aceasta comanda asupra ta.");
...
CMD:set(playerid, params[]) {
	...
	switch(YHash(option)) {
		case _H<money>: {
			if(value < 0 || value > 2000000000) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Result Invalid (0 - 2,000,000,000).");
		...
		case _H<billion>: {
			if(value < 0 || value > 100) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Result Invalid (0 - 100).");
		...
		case _H<respectpoints>, _H<rp>: {
			if(value < 0 || value > 500) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Result Invalid (0 - 500).");
		...
		case _H<level>: {
			if(value < 0 || value > 150) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Result Invalid (0 - 150).");
			if(playerInfo[id][pLevel] == value) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja level %d.", value);			
		...
		case _H<skin>: {
			if(value < 0 || value > 311) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Result Invalid (0 - 311).");
			if(playerInfo[id][pSkin] == value) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja skin %d.", value);			
			setSkin(id, value);
			update("UPDATE `server_users` SET `Skin` = '%d' WHERE `ID` = '%d' LIMIT 1", playerInfo[id][pSkin], playerInfo[id][pSQLID]);					
		}	
		case _H<premium>: {
			if(value < 0 || value > 1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Result Invalid (0 - 1).");
		...
		case _H<premiumpoints>: {
			if(value < 0 || value > 100000) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Result Invalid (0 - 100,000).");
		...
		case _H<virtualworld>: {
			if(value < 0 || value > 2147483647) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Result Invalid (0 - 2,147,483,647).");
			SetPlayerVirtualWorld(id, value);			
		}
		case _H<interior>: {
			if(value < 0 || value > 256) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Result invalid (0 - 256).");
...
CMD:pm(playerid, params[]) {
	if(!Iter_Contains(ServerAdmins, playerid) && !Iter_Contains(ServerHelpers, playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
...
CMD:gotomark(playerid, params[]) {
	if(!Iter_Contains(ServerAdmins, playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(playerInfo[playerid][pMark] == false) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai pozitii salvate, foloseste comanda (/mark) pentru a salva pozitiile.");
...
CMD:area(playerid, params[]) {
	if(playerInfo[playerid][pAdmin] < 2) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	...
	switch(YHash(option)) {
		case _H<disarm>: {
			if(value < 1 || value > 200) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid Range (1 - 200).");
			GetPlayerPos(playerid, x, y, z);		
			foreach(new i : loggedPlayers) {
		...
		case _H<gun>: {
			if(value < 1 || value > 200) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid Range (1 - 200).");
			if(value1 < 1 || value1 > 46) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid Value (1 - 46).");
			GetPlayerPos(playerid, x, y, z);
			foreach(new i : loggedPlayers) {
		...
		case _H<heal>: {
			if(value < 1 || value > 200) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid Range (1 - 200).");
			if(value1 < 0 || value1 > 100) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid Value (0 - 100).");
			GetPlayerPos(playerid, x, y, z);
			foreach(new i : loggedPlayers) {
				if(IsPlayerInRangeOfPoint(i, value, x, y, z) && GetPlayerInterior(playerid) == GetPlayerInterior(i) && GetPlayerVirtualWorld(playerid) == GetPlayerVirtualWorld(i)) {
...
CMD:givegun(playerid, params[]) {
	...
	if(!isPlayerLogged(id)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");
	if(weapon_id < 1 || weapon_id > 46 || weapon_id == 19 || weapon_id == 20 || weapon_id == 21 || weapon_id == 44 || weapon_id == 45) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid Weapon ID.");
	if(weapon_ammo < 1 || weapon_ammo > 999) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid Ammo (1 - 999).");
	if(playerInfo[id][pWeaponLicense] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu detine licenta de port-arma.");
...
CMD:disarm(playerid, params[]) {
	...
	if(!isPlayerLogged(id)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");
	if(!GetPlayerWeapons(id)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu are arme.");
...
CMD:reports(playerid, params[]) {
	if(!Iter_Contains(ServerAdmins, playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(!Iter_Count(Reports)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu sunt report-uri.");
...
CMD:acceptreport(playerid, params[]) {
	if(!Iter_Contains(ServerAdmins, playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(!Iter_Count(Reports)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu sunt report-uri.");
	if(playerInfo[playerid][pReportChat] != INVALID_PLAYER_ID) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai deja o conversatie deschisa.");
	extract params -> new player:id; else return sendPlayerSyntax(playerid, "/acceptreport <name/id>");
	if(!isPlayerLogged(id)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");
	if(!PlayerHaveReport(id)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu are un report activ.");
	if(playerInfo[id][pReportChat] != INVALID_PLAYER_ID) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul are deja o conversatie deschisa.");
...
CMD:rchat(playerid, params[]) {
	if(playerInfo[playerid][pReportChat] == INVALID_PLAYER_ID) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai o conversatie deschisa.");
...
CMD:reportmute(playerid, params[]) {
	...
	if(!isPlayerLogged(id)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");
	if(minutes < 0 || minutes > 240) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Numar de minute Invalid (0 - 240");
	if(minutes == 0) {
		if(playerInfo[id][pReportMute] < gettime()) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu are mute pe report.");
...
/*CMD:createpickup(playerid, params[]) {
	if(playerInfo[playerid][pAdmin] < 6) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(Iter_Free(Pickups) == -1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}In acest moment, nu se pot creea pickup-uri, deoarece s-a atins numarul maxim.");
	if(IsPlayerInAnyVehicle(playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa cobori din vehicul.");
	if(playerInfo[playerid][pFlymode] == true) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti face acest lucru deoarece esti in fly mode.");
	...
	if(worldid < -1 || worldid > 2147483647) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Virtual World Invalid (0 - 2,147,483,647).");
	if(interior < -1 || interior > 255) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Interior Invalid (0 - 255)");
	...
	mysql_format(SQL, gQuery, 256, "INSERT INTO `server_pickups` (Model, X, Y, Z, WorldID, Interior) VALUES ('%d', '%f', '%f', '%f', '%d', '%d')", model, pickupInfo[id][pickupX], pickupInfo[id][pickupY], pickupInfo[id][pickupZ], worldid, interior);
   	inline getSQLPickupID() {
   		if(!cache_affected_rows()) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu s-a putut creea pickup-ul.");
...
CMD:banip(playerid, params[]) {
	if(playerInfo[playerid][pAdmin] < 2) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(GetPVarInt(playerid, "banDeelay") > gettime()) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa astepti %d secunde inainte sa folosesti aceasta comanda.", (GetPVarInt(playerid, "banDeelay") - gettime()));
	extract params -> new player:id, string:reason[64]; else return sendPlayerSyntax(playerid, "/banip <name/id> <reason>");
	if(!isPlayerLogged(id)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");
	if(id == playerid) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti folosi aceasta comanda asupra ta.");
	if(playerInfo[id][pAdmin] >= playerInfo[playerid][pAdmin] && !strmatch(getName(playerid), "Vicentzo")) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti sa-i dai ban acelui jucator.");
	gQuery[0] = (EOS);
...
CMD:unbanip(playerid, params[]) {
	if(playerInfo[playerid][pAdmin] < 2) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(GetPVarInt(playerid, "unbanDeelay") > gettime()) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa astepti %d secunde inainte sa folosesti aceasta comanda.", (GetPVarInt(playerid, "unbanDeelay") - gettime()));
...
CMD:banipoffline(playerid, params[]) {
	if(playerInfo[playerid][pAdmin] < 2) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	if(GetPVarInt(playerid, "banDeelay") > gettime()) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Trebuie sa astepti %d secunde inainte sa folosesti aceasta comanda.", (GetPVarInt(playerid, "banDeelay") - gettime()));
	extract params -> new string:ip[16], string:reason[64]; else return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}/banipoffline <ip> <reason>");
	foreach(new i : loggedPlayers) {
		if(strmatch(getIp(i), ip)) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Exista deja un jucator cu acest ip, nume: %s (id: %d).", getName(i), i);
	}
	gQuery[0] = (EOS);
...
CMD:gotojob(playerid, params[]) {
	...
	if(!Iter_Contains(ServerJobs, strval(params))) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest job nu exista.");
...
CMD:unjail(playerid, params[]) {
	if(playerInfo[playerid][pAdmin] < 3) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda.");
	...
	if(strlen(reason) < 1 || strlen(reason) > 32) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Reason invalid, min. 1 caracter max. 32 caractere.");
	if(!Iter_Contains(JailedPlayers, userID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu are jail.");
...
CMD:unjailo(playerid, params[]) {
	...
	if(strlen(reason) < 1 || strlen(reason) > 32) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Reason invalid, min. 1 caracter max. 32 caractere.");
	foreach(new i : loggedPlayers) if(strmatch(getName(i), playerName)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul este conectat.");
	gQuery[0] = (EOS);
...
CMD:jail(playerid, params[]) {
	...
	if(minutes < 1 || minutes > 500) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Minute invalide, min. 1 min. max. 500 min.");
	if(userID == playerid) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti folosi aceasta comanda asupra ta.");
	if(strlen(reason) < 1 || strlen(reason) > 32) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Reason invalid, min. 1 caracter max. 32 caractere.");
	if(!isPlayerLogged(userID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu este logat.");
	if(Iter_Contains(JailedPlayers, userID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player este deja in jail.");
	new rand = random(6); 
	if(rand == 0) rand = 1;
...
CMD:showfreq(playerid, params[]) {
    ...
    if(playerInfo[userID][pWTalkie] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu are un walkie talkie.");
...
CMD:announce(playerid, params[]) {
	...
	if(type < 0 || type > 2) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid type (0 - 2).");
...
CMD:count(playerid, params[], help) {
	...
	if(!(1 <= count <= 30)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Invalid count (1 - 30).");
	CountTime = count;
...
CMD:acover(playerid, params[]) {
	...
	if(isPlayerLogged(GetPlayerID(name))) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest jucator este connectat.");
	if(strmatch(name, "Vicentzo") || strmatch(name, "mr.bunny")) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti pune acest nume");
...
CMD:gotohouse(playerid, params[]) {
	...
	if(!Iter_Contains(ServerHouses, houseID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Aceasta casa nu exista.");
...
CMD:gotobusiness(playerid, params[]) {
	...
	if(!Iter_Contains(ServerBusinesses, businessID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Aceast business nu exista.");
