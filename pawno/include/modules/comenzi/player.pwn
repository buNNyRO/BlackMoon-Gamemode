CMD:admins(playerid, params[])
{
	if(!Iter_Count(ServerAdmins))
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu sunt admini conectati.");
...
CMD:helpers(playerid, params[])
{
	if(!Iter_Count(ServerHelpers))
		return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu sunt helperi conectati.");
...
CMD:buylevel(playerid, params[])
{
	...
	if(GetPlayerCash(playerid) < money)
		return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai nevoie de $%s.", formatNumber(money));

	new respect = (playerInfo[playerid][pLevel] * 3);
	if(playerInfo[playerid][pRespectPoints] < respect)
		return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai nevoie de %d puncte de respect.", respect);
...
CMD:exam(playerid, params[]) {
	if(playerInfo[playerid][pDrivingLicense] > 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai deja licenta de condus.");
	...
	if(IsPlayerInRangeOfPoint(playerid, 2.5, 1111.0061, -1795.6694, 16.7100) && GetPlayerVirtualWorld(playerid) != 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu te afli la pozitia in care poti da examen-ul, sau te afli in alt Virtual World"); 
	if(IsPlayerInAnyVehicle(playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Te afli intr-un vehicul, coboara jos.");
	if(!Iter_Count(ExamenCheckpoints)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Momentan nu poti sustine acest examen, exista o eroare tehnica te rugam sa revii mai tarziu.");
	if(GetPlayerCash(playerid) < 500) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai $500.");
...
CMD:engine(playerid, params[]) {
	...
	if(vehicle_fuel[GetPlayerVehicleID(playerid)] < 1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest vehicul nu mai are combustibil.");
	if(isBike(GetPlayerVehicleID(playerid))) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest vehicul nu are un motor.");
	if(GetPVarInt(playerid, "engineDeelay") == gettime()) return true;
	if(vehicle_personal[GetPlayerVehicleID(playerid)] > -1) {
		if(personalVehicle[vehicle_personal[GetPlayerVehicleID(playerid)]][pvInsurancePoints] <= 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest vehicul nu detine puncte de asigurare."); SetPVarInt(playerid, "engineDeelay", gettime());
	}
...
CMD:lights(playerid, params[]) {
	if(!IsPlayerInAnyVehicle(playerid) || GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu esti soferul unui vehicul.");
	if(isBike(GetPlayerVehicleID(playerid)) && isPlane(GetPlayerVehicleID(playerid)) && isBoat(GetPlayerVehicleID(playerid))) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest vehicul nu are faruri.");
	new engine, lights, alarm, doors, bonnet, boot, objective;
	GetVehicleParamsEx(GetPlayerVehicleID(playerid), engine, lights, alarm, doors, bonnet, boot, objective);
...
CMD:report(playerid, params[]) {
	if(Iter_Contains(ServerAdmins, playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai acces la aceasta comanda, deoarece esti administrator.");
	...
	if(PlayerHaveReport(playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai deja un report in asteptare.");
...
CMD:killcp(playerid, params[]) {
	if(playerInfo[playerid][pCheckpoint] == CHECKPOINT_NONE) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai un checkpoint activ pe mini map.");
...
CMD:sms(playerid, params[]) {
	if(playerInfo[playerid][pMute] > gettime()) return SCMf(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti folosi aceasta comanda, deoarece ai mute inca %d secunde.", playerInfo[playerid][pMute]-gettime()-gettime());
	extract params -> new phonenumber, string:smstext[180]; else return sendPlayerSyntax(playerid, "/sms <number> <text>");	
	if(playerInfo[playerid][pPhone] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai un telefon.");
	if(playerInfo[playerid][pPhoneOnline] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Telefonul tau este inchis.");
	if(playerInfo[PlayerNumber[phonenumber]][pPhone] == playerInfo[playerid][pPhone]) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu iti poti trimite sms tie.");
	new giveplayerid;
	if(playerInfo[PlayerNumber[phonenumber]][pPhone] == phonenumber && phonenumber != 0) {
		giveplayerid = PlayerNumber[phonenumber];
		if(isPlayerLogged(giveplayerid) && giveplayerid == INVALID_PLAYER_ID) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Numar de telefon invalid.");
		if(playerInfo[giveplayerid][pPhoneOnline] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Telefonul lui este inchis.");
...
CMD:number(playerid, params[]) {
	...
	if(!isPlayerLogged(targetid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu este connectat.");
	if(playerInfo[playerid][pPhoneBook] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai o agenda telefonica.");
	if(targetid == playerid) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti folosi aceasta comanda asupra ta.");
...
CMD:reply(playerid, params[]) {
	...
	if(playerInfo[playerid][pPhone] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai un telefon.");
	if(strlen(playerInfo[playerid][pPhone]) != 4) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai un iPhone.");
	if(playerInfo[playerid][pPhoneOnline] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Telefonul tau este inchis.");
...
CMD:gps(playerid, params[]) {
	if(!isPlayerLogged(playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu esti conectat.");
	if(Dialog_Opened(playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti folosi comanda, deoarece ai un dialog afisat.");
...
CMD:cancel(playerid, params[]) {
	...
	if(strmatch(item, "medic")) {
		if(MedicAcceptedCall[playerid] == -1 && !Iter_Contains(ServiceCalls[SERVICE_PARAMEDICS], playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}You don't have an call.");
	...
	if(strmatch(item, "taxi")) {
		if(TaxiAcceptedCall[playerid] == -1 && !Iter_Contains(ServiceCalls[SERVICE_TAXI], playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}You don't have an call.");
	...
	if(strmatch(item, "instructor")) {
		if(InstructorAcceptedCall[playerid] == -1 && !Iter_Contains(ServiceCalls[SERVICE_INSTRUCTOR], playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}You don't have an call.");
...
CMD:accept(playerid, params[]) {
	...
	if(!isPlayerLogged(targetid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Jucatorul nu este conectat.");
	switch(YHash(item)) {
		case _H<medic>: {
			...
			if(MedicAcceptedCall[playerid] != -1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai deja un apel acceptat.");
			if(vehicleFaction[GetPlayerVehicleID(playerid)] != playerInfo[playerid][pFaction]) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu esti intr-un vehicul al factiunii.");
			if(!Iter_Contains(ServiceCalls[SERVICE_PARAMEDICS], targetid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu are un apel.");
			if(playerInfo[playerid][pCheckpoint] != CHECKPOINT_NONE) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai un checkpoint activ.");
		...
		case _H<taxi>: {
			...
			if(TaxiAcceptedCall[playerid] != -1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai deja un apel acceptat.");
			if(vehicleFaction[GetPlayerVehicleID(playerid)] != playerInfo[playerid][pFaction]) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu esti intr-un vehicul al factiunii.");
			if(!Iter_Contains(ServiceCalls[SERVICE_TAXI], targetid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu are un apel.");
			if(playerInfo[playerid][pCheckpoint] != CHECKPOINT_NONE) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai un checkpoint activ.");
		...
		case _H<instructor>: {
			if(!Iter_Contains(FactionMembers[7], playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu esti in factiunea 'School Instructors' pentru a folosi aceasta comanda.");
			if(InstructorAcceptedCall[playerid] != -1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai deja un apel acceptat.");
			if(vehicleFaction[GetPlayerVehicleID(playerid)] != playerInfo[playerid][pFaction]) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu esti intr-un vehicul al factiunii.");
			if(!Iter_Contains(ServiceCalls[SERVICE_INSTRUCTOR], targetid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu are un apel.");
			if(playerInfo[playerid][pCheckpoint] != CHECKPOINT_NONE) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai un checkpoint activ.");
		...
		case _H<invite>: {
			if(invitedByPlayer[playerid] == -1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai fost invitat de nimeni intr-o factiune.");
		...
		case _H<cinvite>: {
			if(clanInvitedBy[playerid] == -1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai fost invitat de nimeni intr-un clan.");
		...
		case _H<ticket>: {
			if(ticketPlayer[playerid] == -1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai primit o amenda.");
			if(!ProxDetectorS(8.0, playerid, ticketPlayer[playerid])) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu se afla in raza ta.");
		...
		case _H<live>: {
			if(playerInfo[playerid][pLiveOffer] == -1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai primit o invitatie de conversatie 'Live'.");
			if(playerInfo[playerid][pLiveOffer] != targetid) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest player nu ti-a oferit o invitatie de conversatie 'Live'.");
			if(!IsPlayerInVehicle(playerid, GetPlayerVehicleID(playerInfo[playerid][pLiveOffer]))) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu esti in masina cu acest player.");
		...
		case _H<lesson>: {
			if(playerInfo[playerid][pInLesson] > 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Esti deja intr-o lectie.");
			if(playerInfo[targetid][pInLesson] != playerid) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai primit o oferta");
			if(playerInfo[targetid][pInLesson] > 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel instructor are deja o lectie activa.");
			if(playerInfo[targetid][pFaction] != 7) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu face parte din factiunea 'School Instructors'.");
		...
		case _H<license>: {
			if(playerInfo[playerid][pLicenseOffer] == -1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai primit o oferta.");
			if(playerInfo[playerid][pLicenseOffer] != targetid) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu ti-a facut o oferta.");
			if(playerInfo[targetid][pFaction] != 7) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel player nu face parte din factiunea 'School Instructors'.");
			if(playerInfo[playerid][pLicense] == 1) {
				if(PlayerMoney(playerid, 100000)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai aceasta suma de bani.");
				...
				if(PlayerMoney(playerid, 200000)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai aceasta suma de bani.");
				...
				if(PlayerMoney(playerid, 300000)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai aceasta suma de bani.");
...
CMD:carhand(playerid, params[]) {
    if(IsPlayerInAnyVehicle(playerid)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu poti folosi animatii in timp ce esti intr-un vehicul.");
...
CMD:wthelp(playerid, params[]) {
	if(playerInfo[playerid][pWTalkie] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai un walkie talkie.");
...
CMD:setfreq(playerid, params[]) {	
	...
	if(freq < 0 || freq > 10) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Frecvente disponibile: 1-10.");
	if(playerInfo[playerid][pWToggle] == 1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Statia ta a fost oprita. Foloseste comanda /tog pentru a o activa.");
	if(playerInfo[playerid][pWTChannel] == freq) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Esti deja in aceasta frecventa.");
	if(freq == 0) {
...
CMD:wt(playerid, params[]) {
    ...
    if(strlen(msg) < 1 || strlen(msg) > 128) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Mesaj invalid. Minim 1 caracter, maxim 128 caractere.");
	if(playerInfo[playerid][pWToggle] == 1) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Statia ta a fost oprita. Foloseste comanda /tog pentru a o activa.");
	if(playerInfo[playerid][pWTChannel] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu esti pe o frecventa.");
...
CMD:freqmembers(playerid, params[]) {
	if(playerInfo[playerid][pWTalkie] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai un walkie talkie.");
	if(playerInfo[playerid][pWTChannel] == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu esti pe o frecventa.");
	if(Iter_Count(Freqs[playerInfo[playerid][pWTChannel]]) == 0) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu sunt jucatori pe aceasta frecventa.");
...
CMD:finalquest(playerid, params[]) {
	...
	if(playerInfo[playerid][pProgress][0] >= getNeedProgress(playerid, 0) || playerInfo[playerid][pProgress][1] >= getNeedProgress(playerid, 1)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Nu ai misiuni momentan.");
...
CMD:findbusiness(playerid, params[]) {
	if(playerInfo[playerid][pCheckpoint] != CHECKPOINT_NONE) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Ai un checkpoint activ pe minimap.");
	extract params -> new businessID; else return sendPlayerSyntax(playerid, "/findbusiness <business id>");
	if(!Iter_Contains(ServerBusinesses, businessID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acest business nu exista.");
...
CMD:findhouse(playerid, params[]) {
	...
	if(!Iter_Contains(ServerHouses, houseID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acesta casa nu exista.");
	playerInfo[playerid][pCheckpoint] = CHECKPOINT_GPS;
...
CMD:id(playerid, params[]) {
    ...
    if(!isPlayerLogged(userID)) return SCM(playerid, COLOR_ERROR, "[ERROR] {FFFFFF}Acel jucator nu este logat.");
