...
CMD:turfs(playerid, params[]) {
	if(!isPlayerLogged(playerid)) return SCM(playerid, COLOR_ERROR, eERROR"Trebuie sa fi logat pentru a folosi aceasta comanda.");
	if(playerInfo[playerid][pShowTurfs] == 0) {
		...
		for(new i = 1; i < Iter_Count(ServerTurfs); i++) GangZoneHideForPlayerEx(playerid, turfInfo[i][tID]);
