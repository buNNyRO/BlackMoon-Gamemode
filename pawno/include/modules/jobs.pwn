...
CMD:fish(playerid, params[]) {
	...
	if(Fishing[playerid]) return sendPlayerError(playerid, "Deja pescuiesti.");
	if(FishWeight[playerid]) return sendPlayerError(playerid, "Ai deja un peste prins.");
...
CMD:startwork(playerid, params[]) {
	...
	if(playerInfo[playerid][pDrivingLicense] <= 0) return sendPlayerError(playerid, "Nu ai licenta de condus.");
	...
	switch(jobid) {
		...
		case 3: {
			if(!IsPlayerInRangeOfPoint(playerid, 5.5, jobInfo[jobid][jobXST], jobInfo[jobid][jobYST], jobInfo[jobid][jobZST])) return sendPlayerError(playerid, "Nu esti la punctul de start work pentru jobul 'Drugs Dealer'.");
			jobWork(playerid, JOB_DRUGSDEALER);
		}
		case 4: {
			if(!IsPlayerInRangeOfPoint(playerid, 5.5, jobInfo[jobid][jobXST], jobInfo[jobid][jobYST], jobInfo[jobid][jobZST])) return sendPlayerError(playerid, "Nu esti la punctul de start work pentru jobul 'Arms Dealer'.");
			jobWork(playerid, JOB_ARMSDEALER);
		}
		case 5: {
			if(!IsPlayerInRangeOfPoint(playerid, 5.5, jobInfo[jobid][jobXST], jobInfo[jobid][jobYST], jobInfo[jobid][jobZST])) return sendPlayerError(playerid, "Nu esti la punctul de start work pentru jobul 'Carpenter'.");
...
hook OnPlayerEnterCheckpoint(playerid) {
	...
	switch(playerInfo[playerid][pCheckpoint]) {
		...
		case CHECKPOINT_TRUCKERSF: {
			if(!IsTrailerAttachedToVehicle(JobVehicle[playerid])) return sendPlayerError(playerid, "Nu ai atasata remorca de tir.");
